...
def edit_powerpoint_template(template_path, output_path, malaysia, kuala_lumpur, kedah, perak, johor, sarawak, pulau_pinang, sabah, melaka, selangor, 
                             negeri_sembilan, terengganu, pahang, kelantan, max_date, daily_malaysia, daily_kuala_lumpur, daily_kedah, daily_perak, 
                             daily_johor, daily_sarawak, daily_pulau_pinang, daily_sabah, daily_melaka, daily_selangor, daily_negeri_sembilan,
                             daily_terengganu, daily_pahang, daily_kelantan, image1_path, image2_path, image3_path, image4_path, image5_path, 
                             total_donation, unique_donor, max_granular_date):
        ...
        for shape in slide.shapes:
            ...
            if shape.has_text_frame:
                ...
                if "max_date" in shape.text_frame.text:
                    ...
                    set_font_properties(shape.text_frame, 'Verdana', 18, content_font_color, False, PP_ALIGN.LEFT)
                ...
                if "total_donation" in shape.text_frame.text:
                    shape.text_frame.text = shape.text_frame.text.replace("total_donation", "{:,}".format(total_donation))
                    set_font_properties(shape.text_frame, content_font_name, content_font_size, (255, 255, 255), content_is_bold, content_alignment)

                if "unique_donor" in shape.text_frame.text:
                    shape.text_frame.text = shape.text_frame.text.replace("unique_donor", "{:,}".format(unique_donor))
                    set_font_properties(shape.text_frame, content_font_name, content_font_size, (255, 255, 255), content_is_bold, content_alignment)

                if "max_granular_date" in shape.text_frame.text:
                    shape.text_frame.text = shape.text_frame.text.replace("max_granular_date", "{}".format(max_granular_date))
                    set_font_properties(shape.text_frame, 'Verdana', 18, content_font_color, False, PP_ALIGN.LEFT)

            if (hasattr(shape, 'image')): 
                if getattr(shape, 'name', None) == 'Chart 1':
                    print('Replace chart 1')
                    ...
                    new_picture = slide.shapes.add_picture(image1_path, left, top, width, height)
                    new_picture.name = 'Graph 1' 

                if (getattr(shape, 'name', None) == 'Chart 2'):
                    print('Replace chart 2')
                    shape.element.getparent().remove(shape.element)
                    left = shape.left
                    top = shape.top
                    width = shape.width
                    ...
                    new_picture = slide.shapes.add_picture(image2_path, left, top, width, height)
                    new_picture.name = 'Graph 2' 

                
                if getattr(shape, 'name', None) == 'Chart 3':
                    print('Replace chart 3')
                    shape.element.getparent().remove(shape.element)
                    left = shape.left
                    top = shape.top
                    width = shape.width
                    ...
                    new_picture = slide.shapes.add_picture(image3_path, left, top, width, height)
                    new_picture.name = 'Graph 3' 

                if (getattr(shape, 'name', None) == 'Chart 4'):
                    print('Replace chart 4')
                    shape.element.getparent().remove(shape.element)
                    left = shape.left
                    top = shape.top
                    width = shape.width
                    ...
                    new_picture = slide.shapes.add_picture(image4_path, left, top, width, height)
                    new_picture.name = 'Graph 4'

                if (getattr(shape, 'name', None) == 'Chart 5'):
                    print('Replace chart 5')
                    shape.element.getparent().remove(shape.element)
                    left = shape.left
                    top = shape.top
                    width = shape.width
                    ...
                    new_picture = slide.shapes.add_picture(image5_path, left, top, width, height)
                    new_picture.name = 'Graph 4'
    ...
    print(f"Edited pptx saved to {output_path}")
